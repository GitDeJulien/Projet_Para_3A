...
void Update(float *A)
{
   ...
   for(I=0; I<Np; I++){
      if(Common[I]>0){
         MPI_Recv(&Buf[0], Common[I], MPI_FLOAT, I, 100, MPI_COMM_WORLD, &Status);
      }
      for(J=0; J<Common[I]; J++){
...
int main( int argc, char** argv)
{
   ...
   char* OuterFilePath;
   OuterFilePath = argv[1];

   if (ProcID < 10)
      sprintf(FileName, "%sData0%d.In",OuterFilePath, ProcID);
   else
      sprintf(FileName, "%sData%d.In",OuterFilePath, ProcID);
...
/*   for (I=0; I<IntNodes; I++) {
   ...
   sprintf(OutName,"%sSol0%d.plt", OuterFilePath, ProcID);
   ...
   sprintf(TimeName,"%sproc.%d.nbproc.%d.plt", TimeFilePath, ProcID, ProcNo);
