...
!***   are checked
   ...
   &                              (vois(i,m), i = 1,3), itri(1,m)
      ...
      partitionunit = 10
      tecunit = 11
      !open(partitionunit,file='partition_metis_4/dualformetis.dat.epart.4',status='old') !for metis
      open(partitionunit,file='./scotch/proc'//trim(args(1))//'/dualforscotch.map',status='old') !for scotch 
      open(tecunit,file='./visu/decomp_scotch'//trim(args(1))//'.plt',status='unknown')
