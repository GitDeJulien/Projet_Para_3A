...
!***   are checked
   ...
   &                              (vois(i,m), i = 1,3), itri(1,m)
      ...
      partitionunit = 10
      tecunit = 11
      open(partitionunit,file='./metis/proc'//trim(args(1))//&
      '/dualformetis.data.dgraph.part.'//trim(args(1)),status='old') !for metis
      !open(partitionunit,file='partition_scotch_4/dualforscotch.map',status='old') !for scotch 
      open(tecunit,file='./visu/decomp_metis'//trim(args(1))//'.plt',status='unknown')
